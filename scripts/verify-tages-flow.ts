...
check(
  'Erstnachrichten: das Tages-Soll ist die Runde von 50 (25.09.2026), der Deckel bleibt 50',
  sollFuer(TAGES_FLOW[ERSTNACHRICHTEN], eingabe({ erstnachrichtenOffen: 98 })) === ERSTNACHRICHTEN_RUNDE &&
    ERSTNACHRICHTEN_RUNDE === 50 &&
    ERSTNACHRICHTEN_LIMIT_TAG === 50,
)
check(
  'Erstnachrichten: eine schon eingefrorene 98 (oder 50) wird auf die 50 gezogen',
  sollFuer(TAGES_FLOW[ERSTNACHRICHTEN], eingabe({ erstnachrichtenOffen: 98, portionen: { erstnachrichten: 98 } })) === 50 &&
    sollFuer(TAGES_FLOW[ERSTNACHRICHTEN], eingabe({ erstnachrichtenOffen: 98, portionen: { erstnachrichten: 50 } })) === 50,
)
check(
  'Erstnachrichten: nachgeschossene Texte machen die Zeile nicht rot (60 von 50 ist erledigt)',
  (() => {
    const e = eingabe({ erstnachrichtenOffen: 300, erstnachrichtenTexte: 5, today: { li_nachrichten: 60 } })
    const stand = stufenStaende(e)[ERSTNACHRICHTEN]
    return stand.soll === 50 && stand.wert === 60 && stand.erledigt
  })(),
)
check(
